...
Пример в Go: Предположим, мы хотим создать логгер, который будет использоваться в приложении только один раз.
...
Особенности в Go: Вместо классов и приватных конструкторов Go использует пакеты и sync.Once для гарантированной инициализации единственного экземпляра. 
Это позволяет избежать лишней сложности.
______________________________________________________________________________________________
2. Factory (Фабрика)
Описание: Создаёт объекты без явного указания их конкретного класса. 
В Go фабричные функции часто используются для создания сложных структур или интерфейсов.
Пример в Go: Предположим, мы хотим создавать разные типы транспортных средств (например, автомобили, самолёты).
...
Описание: Позволяет объекту уведомлять других объектов о изменениях состояния. 
В Go это часто реализуется через каналы (channels) или интерфейсы.
Пример в Go: Предположим, у нас есть новостная система, где подписчики получают уведомления.
...
Особенности в Go: Go использует интерфейсы и композицию вместо сложных наследований. 
Каналы также могут быть альтернативой для передачи уведомлений в реальном времени.
______________________________________________________________________________________________
4. Decorator (Декоратор)
Описание: Позволяет динамически добавлять ответственность к объекту, обёртывая его в другой объект. 
В Go это часто реализуется через композицию структур.
Пример в Go: Предположим, мы хотим добавить функциональность к кофе (например, молоко, сахар).
...
Описание: Позволяет определять семейство алгоритмов, инкапсулировать каждый из них и делать их взаимозаменяемыми. 
Клиентский код может выбирать алгоритм во время выполнения без изменения структуры программы.
Пример в Go: Предположим, у нас есть система оплаты, где мы можем использовать разные стратегии оплаты (наличные, кредитная карта, PayPal).
...
Особенности в Go: Strategy реализуется через интерфейс (PaymentStrategy) и структуры, которые реализуют этот интерфейс. 
Это позволяет легко добавлять новые стратегии без изменения существующего кода.
______________________________________________________________________________________________
6. Adapter (Адаптер)
Описание: Позволяет объектам с несовместимыми интерфейсами работать вместе, адаптируя интерфейс одного объекта под другой.
Пример в Go: Предположим, у нас есть старый сервис уведомлений с несовместимым интерфейсом, и мы хотим адаптировать его под новый интерфейс уведомлений.
...
Особенности в Go: Адаптер реализуется через композицию (встраивание старого сервиса в новую структуру) и реализацию нового интерфейса. 
Go не требует сложных наследований, что упрощает создание адаптеров.
______________________________________________________________________________________________
7. Proxy (Прокси)
Описание: Предоставляет замену реальному объекту, чтобы контролировать доступ к нему,
добавлять дополнительную логику (например, кэширование, ленивую загрузку, безопасность).
Пример в Go: Предположим, у нас есть сервис изображений, и мы хотим добавить прокси для кэширования запросов.
